...
fun String.toTitleCase(): String {
  return this.split(".", "-")
     ...
     .joinToString(" ") { if (acronyms.contains(it)) it.uppercase() else it.replaceFirstChar { c -> c.uppercase() } }
}
...
val String.beanPropertyName: String get() {
  val parts = split("-", ".")
  return (parts.take(1) + parts.drop(1).map { it.replaceFirstChar { c -> c.uppercase() } }).joinToString("")
}
