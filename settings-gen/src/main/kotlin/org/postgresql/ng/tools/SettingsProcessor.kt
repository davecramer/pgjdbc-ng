...
import org.postgresql.ng.tools.SettingsProcessor.Companion.SETTING_TYPE_NAME
...
class SettingsProcessor : AbstractProcessor() {
  ...
  companion object {
    ...
    private const val PG_PKG = "org.postgresql.ng"
    ...
    OutputStreamWriter(FileOutputStream(file))
       .use { out ->
         ...
         for (group in orderedGroups) {
           ...
           val isDSGroup = group.id == JDBC_DS_GROUP_NAME
           val rowsPerEntry = if (isDSGroup) 2 else 3

           out.append("== ").append(group.desc).append("\n\n")

           out.append(
              "[cols=\"4a,3a,2\"options=header]\n" +
                 "|===\n" +
           ...
           for (setting in settings) {
             ...
             fun appendDriverProperty() {
               out.append("|.Driver\n")
                  .append("`").append(setting.name).append("`")
                  .append("\n\n")
             }

             fun appendDataSourceProperty() {

               val typeName = ClassName.get(setting.type)
               val simpleTypeName = (typeName as? ClassName)?.simpleName() ?: typeName.toString()
             ...
             fun appendSystemProperty() {

               out.append("|.System\n")
                  .append("`").append(SYSTEM_PROPERTY_PREFIX).append(setting.name).append("`")
                  .append("\n\n")
             }

             out.append(".$rowsPerEntry+|[[").append(setting.name.replace('.','_')).append("]]")
                .append("*").append(setting.name.toTitleCase()).append("*\n\n")

             out.append(setting.desc.toAsciiDoc().escapeAsciiDoc())
                .append("\n\n")

             if (setting.type.isEnum) {
               val enumElement = (setting.type as? DeclaredType)!!.asElement() as TypeElement
               out.append(enumAsciiDoc(enumElement))
                  .append("\n\n")
             }

             if (!isDSGroup) {
               appendDriverProperty()
             }
             else {
               appendDataSourceProperty()
             }

             var appendRange: (() -> Unit)? = null

             if (setting.dynamicDefaultCode == null && setting.staticDefaultCode == null) {

               val rangeDesc =
                  if (setting.min != null && setting.max != null)
                    "Must be between `${setting.min}` and `${setting.max}`"
                  else if (setting.min != null)
                    "Must be greater than or equal to `${setting.min}`"
                  else if (setting.max != null)
                    "Must be less than or equal to `${setting.max}`"
                  else
                    null

               rangeDesc?.let {
                 appendRange = {
                   out.append(".${rowsPerEntry - 1}+a|")
                      .append(rangeDesc)
                      .append("\n\n")
                 }
               }
               ...
               out.append(".$rowsPerEntry+a|")
                  .append(setting.default.toAsciiDoc())
                  .append("\n\n")
             }
  ...
  private fun generateDataSourceWithProperties(groups: Set<GroupInfo>, settings: Set<SettingInfo>): Boolean {
    ...
    for (setting in settings) {
      ...
      val setter = MethodSpec.methodBuilder("set" + setting.name.beanPropertyName.replaceFirstChar { it.uppercase() })
         ...
         .apply {
           if (setting.type.isEnum) {
             val enumElement = (setting.type as? DeclaredType)!!.asElement() as TypeElement
             addJavadoc(enumJavadoc(enumElement) + "\n\n")
           }
         }
    ...
    JavaFile.builder("org.postgresql.ng.jdbc", dsClass)
       .skipJavaLangImports(true)
       .build()
       .writeTo(filer)
  ...
  private fun enumJavadoc(element: TypeElement): String {
  ...
  private fun enumAsciiDoc(element: TypeElement): String {
    ...
    val map = element.enclosedElements.filter { it.kind == ElementKind.ENUM_CONSTANT }
       .map { enumConst ->

         val value = enumConst.simpleName.toString()

         val descriptionAnn = enumConst.annotationMirrors.firstOrNull { it.annotationType == descriptionAnnType }
         val description =
            if (descriptionAnn != null) {
              (descriptionAnn.elementValues.values.first().value as String).let { if (it.isBlank() || it.isEmpty()) null else it }
            }
            else
              null

         valueFormatter(value) to description
       }
       .toMap()

    val anyDescriptions = map.any { it.value != null }
       ...
       .use { out ->
         ...
         if (anyDescriptions) {
           ...
           map.forEach { (name, desc) ->
             out.append("$name:: ${desc?.toAsciiDoc() ?: ""}\n\n")
           }
